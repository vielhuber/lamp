...
| `./lamp build [<id\|subdomain>] [<args>...]`                                       | rerun the configured build; without an id or subdomain, use the environment containing the current directory; further arguments reach the build script as `"$@"`                                                                                  |
...
| `./lamp exec [<id\|subdomain>] "<command>"`                                        | run in the specified or automatically detected project with its php, setup variables and authenticated `curl`; outside registered projects: as root in the container                                                                                                                    |
...
| `./lamp ssh [<id\|subdomain>]`                                                     | interactive root shell in the container; with an environment: in its project directory with its variables, `git status` first if `.git` exists                                                                                                                     |
...
| `postfix.oauth_tenant_id`, `postfix.oauth_client_id`, `postfix.oauth_client_secret` | unset                               | Office365 application credentials instead of `password`; requires `username` and `relayhost`, uses XOAUTH2 with verified TLS and refreshes the token every five minutes             |
| `postfix.sender`                                                                    | unchanged                           | optional address used for all envelope and header senders, including system mail; recipients and Reply-To stay unchanged                                                            |
...
- `start`, `restart`, `cloudflare-setup`, `build`, `syncdb` and `add` mirror the repository first (`git fetch` and `git reset --hard` with `ssh/id_rsa` of the clone), so a pushed build script is used by the next `./lamp build <id>` without a restart; failed fetches (including DNS errors) are retried up to five attempts with five seconds between attempts. If all attempts fail, the command aborts without resetting the clone or continuing with stale data
...
- `./lamp audit` checks immediate Git checkouts in `/var/www` inside the running container. GitHub projects whose `origin` belongs to neither `vielhuber` nor one of the authenticated user's organizations (such as `phpmyadmin/phpmyadmin`) are excluded from the project count, checks, fetches and automatic pulls. Both public and private `vielhuber` and organization repositories remain included; repositories without a recognized GitHub origin are still checked. It discovers organizations with [`gh org list`](https://cli.github.com/manual/gh_org_list) and compares local checkouts with `gh repo list --no-archived` for `vielhuber` and every organization (excluding `setup` and the profile repository `vielhuber`). Repositories are matched by their Git `origin` URL, independently of local folder names; GitHub SSH and HTTPS URLs are normalized to the same repository identity. It reports missing clones and folders without Git only when found; LAMP's internal `_environments` directory is excluded from the missing-Git check. Archived repositories are excluded from the missing-clone check. Hidden folders and Git worktrees are included; `_archive` is skipped for per-project checks. It reads static environments from `/etc/lamp-config/env.yaml` and reports `lamp missing` per project.
- audit clears an interactive terminal before printing its results; redirected output contains no clear-screen sequences. It fetches each upstream with a 60-second timeout, then runs `git pull --ff-only --no-rebase --no-autostash` only when the working tree has no tracked, staged or untracked changes and remote commits are available. It recalculates ahead/behind afterwards. Diverged histories and failed fetches/pulls remain flagged. Nothing is committed, pushed, cloned or built.
...
- Playwright is installed globally with Chromium, its headless shell and system dependencies for tools such as `critical` 9. Playwright versions require matching browser binaries; after upgrading a project's Playwright version, run its local `npx playwright install chromium` if a different browser revision is required.
...
- `lamp build`, `lamp exec <id>` and `lamp ssh <id>` preload `/opt/lamp/node-access.cjs` through `NODE_OPTIONS`. Node `fetch` GET/HEAD requests and Playwright `chromium.launch()` automatically use the same service token, including `critical`'s initial HTML fetch and render engine. Authentication is restricted to the environment's exact HTTPS origins and aliases; every redirect is checked again, and foreign origins never receive the injected headers. Browser interception also covers assets, frames and service workers. Token values stay in process memory and are hidden from Playwright request-header inspection; they are not exported into shell variables or written into project files. Plain `lamp ssh` without an environment does not enable this integration.
- test the Node/browser integration inside LAMP with `node --test /var/www/lamp/docker/tests/test_node_access.cjs`; it uses local test servers and fixture credentials. Existing environment shells must reconnect after installing this integration.
...
- `lamp exec [<id|subdomain>] "npm test"` runs the command in the checkout with the right php and database variables. Without an id, it selects the environment containing the current directory (including subfolders); the closest project root wins. Ambiguous matches require an explicit id or subdomain. Outside registered projects, it keeps the general container shell.
...
- `./lamp ssh` is `docker exec`; no ssh server port is published; `./lamp ssh <id|subdomain>` sources the environment's `setup.env`, changes into the project and runs `git status` before the shell when `.git` exists (directory or worktree file)
