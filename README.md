...
- chat environments get `cache: 360` and `threads` (number of cores, at most 16) unless the profile sets them: the dump is fetched at most every six hours, kept in the state volume, and the tables are restored by several clients at once (syncdb 2.2); static environments import as the profile says
