...
- chat environments get `cache: 6h` on the source, `threads: true` on the target and `cache_dir` in the state volume unless the profile sets them: the production dump is fetched at most every six hours and the tables are restored by several clients at once (syncdb 2.2); static environments import as the profile says
