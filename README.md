...
| `syncdb.domains`                       | empty                               | domains of the other lamp instances that share the syncdb profiles; their hostnames in `replace` rules are rewritten for dynamic environments like the own domain               |
...
- `syncdb <profile>` inside a build or `exec <id>` copies the profile, replaces its complete `target` with the environment's database (static: the fixed `db_name` as root, or the sqlite file; dynamic: the isolated `lamp_<id>` or sqlite file), imports with php 8.5 in a temporary directory, deletes the copy; `source` stays unchanged; `replace` stays unchanged for static environments, in dynamic environments every `<label>.<domain>` hostname in its keys and values becomes the environment's own hostname, so a profile written for the static site works for its chat environments, also on another instance when the domain it was written for is listed in `syncdb.domains`; the profile engine must match `db_engine`
